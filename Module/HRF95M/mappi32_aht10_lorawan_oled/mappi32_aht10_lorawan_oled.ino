...
#define SCREEN_WIDTH 128 // Lebar layar OLED Anda (sesuaikan sesuai dengan layar Anda)
#define SCREEN_HEIGHT 64 // Tinggi layar OLED Anda (sesuaikan sesuai dengan layar Anda)
#define SCREEN_ADDRESS 0x3C ///< See datasheet for Address; 0x3D for 128x64, 0x3C for 128x32
Adafruit_SSD1306 display(SCREEN_WIDTH, SCREEN_HEIGHT, &Wire, OLED_RESET); // Ubah alamat I2C sesuai dengan layar Anda
...
void onEvent (ev_t ev) {
  ...
  switch (ev) {
    ...
    case EV_TXCOMPLETE:
      ...
      display.setCursor(0, display.height() - 8); // Adjust the 8 to your desired text height
      display.println("Packet Send Succes!");
      display.display();
      delay(2000);
      display.clearDisplay();
      display.display();
    ...
    default:
      ...
      display.setTextColor(SSD1306_WHITE);
      display.setCursor(0, display.height() - 8); // Adjust the 8 to your desired text height
      display.println("Packet Send Failure");
      display.display();
      break;
...
void do_send(osjob_t* j) {
  ...
  if (LMIC.opmode & OP_TXRXPEND) {
    ...
    sendData();
      ...
      // Add code here to handle the failure (e.g., display it on OLED)
...
void setup() {
  ...
  LMIC_setSession (0x1, DEVADDR, NWKSKEY, APPSKEY);
    ...
    // Setup channels for AS923 region
    ...
    LMIC_setupChannel(8, 924800000, DR_RANGE_MAP(DR_FSK, DR_FSK), BAND_MILLI);     
